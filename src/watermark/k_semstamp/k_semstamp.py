...
from ..exceptions import AlgorithmNameMismatchError
...
class KSemStampConfig:
    ...
    def __init__(self, algorithm_config: str, transformers_config: TransformersConfig, *args, **kwargs) -> None:
        ...
        if algorithm_config is None:
            config_dict = load_config_file('config/KSEMSTAMP.json')
        else:
            config_dict = load_config_file(algorithm_config)
        if config_dict['algorithm_name'] != 'KSEMSTAMP':
            raise AlgorithmNameMismatchError('KSEMSTAMP', config_dict['algorithm_name'])

        self.domain_data = config_dict['domain_data']
        self.max_new_tokens = config_dict['max_new_tokens']
        self.min_new_tokens = config_dict['min_new_tokens']
        self.path_to_embedder = config_dict['path_to_embedder']
        self.N_max = config_dict['N_max']
        self.gamma = config_dict['gamma']
        self.margin_m = config_dict['margin_m']
        self.k = config_dict['k']
        self.prime_P = config_dict['prime_P']
        self.threshold = config_dict['threshold']
        path_to_centroids = config_dict['path_to_centroids']
        # Resolve relative path relative to Topic-Based-Watermarks/src (parent of config dir)
        if not os.path.isabs(path_to_centroids):
            config_dir = os.path.dirname(os.path.abspath(algorithm_config))
            src_root = os.path.dirname(config_dir)  # parent of config/ is src/
            path_to_centroids = os.path.join(src_root, path_to_centroids)
...
class KSemStampUtils:
    ...
    def generate_cluster(self):
        ...
        embeds = self.load_embeds(embed_path)
...
class KSemStamp(BaseWatermark):
    ...
    def __init__(self, algorithm_config: Union[str, KSemStampConfig], transformers_config: Optional[TransformersConfig] = None, *args, **kwargs) -> None:
        """
            Initialize the KSEMSTAMP algorithm.

            Parameters:
                algorithm_config (Union[str, KSemStampConfig]): Path to the algorithm configuration file or KSemStampConfig instance.
                transformers_config (Optional[TransformersConfig]): Configuration for the transformers model.
        """
